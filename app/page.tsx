...
export default function HomePage() {
  ...
  // Components to avoid Rules of Hooks violation in loops
  const CampCard = ({ camp, t }: { camp: any; t: any }) => {
    const imageSrc = getPrimaryImage(camp.images);
    const translatedName = getLocalizedField(camp, "name", i18n.language);
    const translatedLocation = getLocalizedField(camp, "location", i18n.language);
    const translatedPrice = useTranslatedPrice(`camp[${camp.id}].price`, camp.pricePerNight, "MNT");
      ...
      >
        <div className="flex flex-col h-full">
          <div className="relative h-40 sm:h-52 w-full overflow-hidden">
            ...
            <div className="absolute top-2 left-2 right-2 flex flex-col gap-1.5 items-start sm:flex-row sm:justify-between sm:items-center z-20 pointer-events-none">
              {camp.isFeatured && (
  ...
  const ProductCard = ({ product, addToCart, toast, t }: { product: any; addToCart: any; toast: any; t: any }) => {
    const imageSrc = getPrimaryImage(product.images);
    const translatedName = getLocalizedField(product, "name", i18n.language);
    const translatedPrice = useTranslatedPrice(`prod[${product.id}].price`, product.price, "MNT");
          ...
          <div className="max-w-7xl 2xl:max-w-[1600px] 3xl:max-w-[2000px] 4k:max-w-[2800px] mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 3xl:grid-cols-6 4k:grid-cols-8 gap-2 sm:gap-4 md:gap-5 lg:gap-6 mb-6">
              {yurts
                ...
                .map((camp: any) => (
                  <CampCard key={camp.id} camp={camp} t={t} />
                ))}
            </div>
