...
export default function ProductDetailPage({ params }: ProductDetailPageProps) {
  ...
  // Consolidated hooks
  const allProductsLabel = useTranslatedValue("products.all_products", "Бүх бүтээгдэхүүн")
  const lowStockLabel = useTranslatedValue("products.low_stock", "Цөөн үлдсэн")
  const ordersLabel = useTranslatedValue("products.orders", "Захиалга")
  const descriptionHeaderLabel = useTranslatedValue("products.description", "Бүтээгдэхүүний тайлбар")
  const naturalLabel = useTranslatedValue("products.features.natural", "100% Байгалийн")
  const naturalDescLabel = useTranslatedValue("products.features.natural_desc", "Цэвэр монгол бүтээгдэхүүн")
  const qualityLabel = useTranslatedValue("products.features.quality", "Чанартай")
  const qualityDescLabel = useTranslatedValue("products.features.quality_desc", "Малчдын гараар бүтсэн")
  const stockStatusLabel = useTranslatedValue("products.stock_status", "Найдвартай")
  const fastDeliveryLabel = useTranslatedValue("products.fast_delivery", "Хурдан хүргэлт")
  const safeShippingLabel = useTranslatedValue("products.safe_shipping", "Найдвартай")
  
  const addedToCartLabel = useTranslatedValue("common.added_to_cart", "сагсанд")
  const addedSuffixLabel = useTranslatedValue("common.added_suffix", "нэмэгдлээ")

  // Dynamic ones
  const productCategoryLabel = useTranslatedValue(`cat.${product?.category?.name || "other"}`, product?.category?.name || "Бараа")
  ...
  const handleAddToCart = () => {
    ...
    toast({
      title: successLabel,
      description: `${translatedName} ${addedToCartLabel} ${quantity} ${pieceLabel} ${addedSuffixLabel}`,
    })
  }
          ...
          <div className="lg:col-span-12 xl:col-span-5 lg:pl-4 xl:pl-0 flex flex-col items-start h-full">
            <div className="w-full">
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className="text-gray-400 text-[10px] font-black uppercase tracking-widest">4.9 • 100+ {ordersLabel}</span>
              </div>
              ...
              <div className="space-y-8 mb-12">
                <div>
                  <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#0F3D2E]/50 mb-4">{descriptionHeaderLabel}</h4>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-2xl bg-white border border-[#0F3D2E]/5 shadow-sm">
                        ...
                        <h5 className="text-xs font-black uppercase tracking-wider text-[#0F3D2E]">{naturalLabel}</h5>
                        <p className="text-[10px] text-gray-400 font-medium mt-1">{naturalDescLabel}</p>
                    </div>
                    <div className="p-4 rounded-2xl bg-white border border-[#0F3D2E]/5 shadow-sm">
                        <Shield className="w-5 h-5 text-emerald-600 mb-2" />
                        <h5 className="text-xs font-black uppercase tracking-wider text-[#0F3D2E]">{qualityLabel}</h5>
                        <p className="text-[10px] text-gray-400 font-medium mt-1">{qualityDescLabel}</p>
                    </div>
                </div>
              </div>
            </div>
            ...
            <div className="w-full mt-auto pt-8 border-t-2 border-[#0F3D2E]/5 space-y-8">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#0F3D2E]/50 mb-3">{stockStatusLabel}</h4>
              ...
              <div className="bg-white/60 p-6 rounded-3xl border border-[#0F3D2E]/5 flex items-center justify-around">
                <div className="text-center group cursor-default">
                    ...
                    <span className="block text-[10px] font-black uppercase tracking-tighter text-[#0F3D2E]">{fastDeliveryLabel}</span>
                </div>
                ...
                <div className="text-center group cursor-default">
                    ...
                    <span className="block text-[10px] font-black uppercase tracking-tighter text-[#0F3D2E]">{safeShippingLabel}</span>
                </div>
              </div>
            </div>
          </div>
