  ...
  return (
    ...
    >
      ...
      <div className="overflow-auto w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 p-2 gap-x-2 gap-y-1">
        {!showProvinceInfo &&
          locationData.zipcode.filter(loc => ["Архангай", "Өвөрхангай"].includes(loc.mnname)).map((loc) => (
            <div key={loc.mnname}>
...
function showProvinceInfoComp(
  ...
  return (
    <div className="w-full p-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-2 gap-y-1">
      {(province?.sub_items || []).filter(item => item.mnname && (item.mnname.includes("Цэнхэр") || item.mnname.includes("Хужирт"))).map((item) => {
        return (
          <div key={item.mnname}>
            <button
